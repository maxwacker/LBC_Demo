...
final class ProductRecordTests: XCTestCase {
    ...
    func test_GIVEN_JsonProductDefinition_WHEN_Decoding_THEN_EntityProperlyInited() {
        ...
        do {
            ...
            XCTAssertEqual(testedProductRecord.id,
                           1461267313)
            XCTAssertEqual(testedProductRecord.category,
                           .home)
            XCTAssertEqual(testedProductRecord.title,
                           "Statue homme noir assis en plâtre polychrome")
            XCTAssertEqual(testedProductRecord.description,
                           "Magnifique Statuette homme noir assis fumant le cigare en terre cuite et plâtre polychrome réalisée à la main.  Poids  1,900 kg en très bon état, aucun éclat  !")
            XCTAssertEqual(testedProductRecord.price,
                           Decimal(140.00))
            XCTAssertEqual(testedProductRecord.imageRef.smallURL,
                           "https://raw.githubusercontent.com/leboncoin/paperclip/master/ad-small/2c9563bbe85f12a5dcaeb2c40989182463270404.jpg" )
            XCTAssertEqual(testedProductRecord.creationDate,
                           ISO8601DateFormatter().date(from: "2019-11-05T15:56:59+0000"))
            XCTAssertEqual(testedProductRecord.isUrgent,
                           false)
